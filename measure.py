...
def metrics(im1, im2, use_GT_mean, loss_fn):
    
    if not next(loss_fn.parameters()).is_cuda:
        loss_fn = loss_fn.cuda()
    im1 = im1[0].permute(1, 2, 0).cpu().numpy() * 255.0
    im2 = im2[0].permute(1, 2, 0).cpu().numpy() * 255.0
